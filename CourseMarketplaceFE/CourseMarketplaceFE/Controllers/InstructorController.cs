...
public class InstructorController : Controller
{
    ...
    // ═══════════════════════════════════════════════════════════════════
    // 8. INSTRUCTOR REPORTS — GET: /Instructor/Reports
    // ═══════════════════════════════════════════════════════════════════
    [HttpGet]
    ...
    public async Task<IActionResult> Reports()
    {
        ...
        var allReports = new List<CourseReportDetailViewModel>();
        var jsonOptions = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };

        try
        {
            // Lấy danh sách khóa học của Instructor
            var resp = await _api.GetAsync("courses/my-courses?page=1&pageSize=100");
            if (resp.IsSuccessStatusCode)
            {
                var json = await resp.Content.ReadAsStringAsync();
                using var doc = JsonDocument.Parse(json);
                if (doc.RootElement.TryGetProperty("data", out var dataEl) &&
                    dataEl.TryGetProperty("courses", out var coursesEl))
                {
                    var tasks = new List<Task<HttpResponseMessage>>();
                    foreach (var item in coursesEl.EnumerateArray())
                    {
                        int courseId = item.GetProperty("courseId").GetInt32();
                        tasks.Add(_api.GetAsync($"/api/report/instructor/courses/{courseId}"));
                    }
                    ...
                    foreach (var r in responses)
                    {
                        if (r.IsSuccessStatusCode)
                        {
                            ...
                            using var rDoc = JsonDocument.Parse(content);
                            if (rDoc.RootElement.TryGetProperty("data", out var reportsEl))
                            {
                                var list = JsonSerializer.Deserialize<List<CourseReportDetailViewModel>>(reportsEl.ToString(), jsonOptions);
                                if (list != null)
                                {
                                    allReports.AddRange(list);
                                }
                            }
                        }
                    }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Error loading instructor reports: {ex.Message}");
        }
    ...
    [HttpGet]
    [Route("Instructor/Reports/GetPendingCount")]
                    ...
                    foreach (var r in responses)
                    {
                        if (r.IsSuccessStatusCode)
                        {
                            ...
                            if (rDoc.RootElement.TryGetProperty("data", out var reportsEl))
                            {
                                ...
                                if (list != null)
                                {
                                    pendingCount += list.Count(x => x.Status?.ToLower() == "pending");
                                }
                            }
                        }
                    }
