...
*   **Database Changes (Insert, Update, Delete):**
    ...
    *   Within the failed branch (`numberOfRowsAffected == 0`), throw `InvalidOperationException` (at the service layer), the controller catches the exception and returns an error `statusCode 400`, and the view displays an error message to the actor.
    *   Explicitly state or report this assumption in your documentation.
*   **Database queries (Select):**
    *   **Query for a single object** (e.g., get course details):
        *   At the service layer, after receiving the return value from the repository, perform a null check and create an `alt` branching.
        *   Within the successful branch (not null), detail the rest of the normal execution path.
        *   Within the failed branch (null), throw `InvalidOperationException`, controllers return `statusCode 404`, and the view displays an error message to the actor.
        *   *Note:* Even if the code performs the null check at the controller layer, ignore that implementation and follow this instruction (model it at the service layer).
    *   **Query for a collection of objects** (e.g., get a list of courses):
        *   At the service layer, after receiving the return value from the repository, perform an empty check and create an `alt` branching.
        *   Within the successful branch (list not empty), detail the rest of the normal execution path.
        *   Within the failed branch (list empty), return the empty list to controllers (backend, frontend), the controllers return `statusCode 200` like usual, and the view displays a "not found" / empty-state message to the actor.
