...
# Dynamically point to the unit_test_docs folder (parent of the script folder)
...
for filepath in md_files:
    ...
    # Split by the standard Function header
    ...
    for i in range(1, len(sections), 2):
        ...
        # 1. Extract Description (Test Requirement)
        desc = ""
        # Support for the current HTML format in TestDocs
        ...
        if html_desc_match:
            ...
            # Fallback for the older Markdown format
        ...
        for line in body.split('\n'):
            if '| **Condition** | **Precondition** |' in line:
                ...
                continue
            if in_precondition:
                # Stop when we hit the next subcategory or category
                ...
                if len(parts) > 3:
                    detail_1 = parts[3]
                    # Make sure it's not empty and not the header name
