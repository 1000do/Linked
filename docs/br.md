...
| BR-171 | **Specific Reason Transparency** All auto-rejection notifications must clearly display specific numeric criteria and metrics for the reject reason to ensure transparency and fairness to the user. The specific descriptions displayed are: \- Account flags limit: "your account having {count} warning flags (limit: 3)" \- Refund frequency limit: "having requested {count} refunds within the last 14 days (limit: 3)" \- Repeated refund: "previous refund history for this course ({count} previous refund)" \- Progress limit for short course: "learning progress of {progress}% exceeding the 15% limit for short courses" \- Watch time limit for long course: "video watch time of {watchHours} hours exceeding the 1.0 hour limit allowed for long courses" |
